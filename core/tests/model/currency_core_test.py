...
def xtest_get_rate_on_empty_db():
    # When there is no data available, use the start_rate.
    # todo: reinstate fallbacks or remove them
...
def test_get_rate_with_daily_rate():
    ...
    setup_daily_rate()
    assert_almost_equal(CAD.value_in(USD, date(2008, 4, 20)), 0.996115, places=6)

def xtest_get_rate_different_currency():
    # Use fallback rates when necessary.
    # todo: reinstate fallbacks or remove them
    setup_daily_rate()
    assert_almost_equal(CAD.value_in(EUR, date(2008, 4, 20)), 1 / EUR.latest_rate, places=6)
    assert_almost_equal(EUR.value_in(USD, date(2008, 4, 20)), EUR.latest_rate * 0.996115, places=6)

def test_get_rate_reverse():
    # It's possible to get the reverse value of a rate using the same data.
    setup_daily_rate()
    assert_almost_equal(USD.value_in(CAD, date(2008, 4, 20)), 1 / 0.996115, places=6)
...
def test_set_rate_twice():
    ...
    assert_almost_equal(CAD.value_in(USD, date(2008, 4, 20)), 42, places=2)
...
def test_set_rate_after_get_the_day_after():
    ...
    USD.set_CAD_value(1/42, date(2008, 4, 20))
    assert_almost_equal(CAD.value_in(USD, date(2008, 4, 21)), 42, places=2)
...
def test_seek_rate_middle():
    ...
    setup_two_daily_rate()
    assert_almost_equal(USD.value_in(CAD, date(2008, 4, 24)), 1/0.996115, places=6)
...
def test_seek_rate_after():
    ...
    setup_two_daily_rate()
    assert_almost_equal(USD.value_in(CAD, date(2008, 4, 26)), 1/0.997115, places=6)

def test_seek_rate_before():
    # If there are no rate in the past, seek for a rate in the future.
    setup_two_daily_rate()
    assert_almost_equal(USD.value_in(CAD, date(2008, 4, 19)), 1/0.996115, places=6)
...
def test_get_rate_multiple_currencies():
    ...
    assert_almost_equal(CAD.value_in(USD, date(2008, 4, 20)), 0.996115, places=6)
    assert_almost_equal(CAD.value_in(EUR, date(2008, 4, 20)), 0.633141, places=6)
...
def test_get_rate_doesnt_exist():
    ...
    setup_rates_of_multiple_currencies()
    USD.value_in(PLN, date(2008, 4, 20)) # no crash
