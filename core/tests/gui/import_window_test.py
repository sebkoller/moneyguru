...
def test_MMM_date_formats_are_supported(app):
    ...
    app.iwin = app.fake_import('foo', TXNS)
...
def test_remember_target_account_selection(app):
    # When selecting a target account, it's specific to the pane we're in
    app.drsel.set_date_range(YearRange(date(2007, 1, 1)))
    app.add_account('foo')
    app.add_account('bar')
    app.iwin = app.mw.parse_file_for_import(testdata.filepath('qif/checkbook.qif'))
    ...
    app.iwin.selected_target_account_index = 2
...
def test_import_selected_pane_with_some_entries_disabled(app):
    ...
    app.iwin.import_table[0].will_import = False
    app.iwin.import_selected_pane()
    app.show_nwview()
    app.bsheet.selected = app.bsheet.assets[0]
    app.show_account()
...
def test_invert_amounts(app):
    ...
    eq_(app.iwin.import_table[1].amount_import, '-100.00')
    eq_(app.iwin.import_table[2].amount_import, '60.00')
...
def test_switch_description_payee(app):
    ...
    eq_(app.iwin.import_table[3].description_import, 'Hydro-Quebec')
    eq_(app.iwin.import_table[3].payee_import, 'Power Bill')
...
def test_match_then_import(app):
    ...
    app.iwin.import_table.bind(2, 5) # second entry --> 04/02/2007 Transfer 80.00
    app.iwin.import_selected_pane()
...
def app_load_with_ref():
    ...
    app.iwin = app.fake_import('foo', app.TXNS)
    app.iwin.import_selected_pane()
    return app
...
def app_import_accountless_qif():
    app = TestApp()
    app.drsel.set_date_range(YearRange(date(2007, 1, 1)))
    app.iwin = app.mw.parse_file_for_import(testdata.filepath('qif/accountless.qif'))
    return app
...
def app_import_accountless_qif_with_splits():
    app = TestApp()
    app.drsel.set_date_range(YearRange(date(2008, 1, 1)))
    app.iwin = app.mw.parse_file_for_import(testdata.filepath('qif/accountless_with_splits.qif'))
    return app
...
def test_transfers(app):
    ...
    eq_(app.iwin.import_table[0].transfer_import, 'Payment Sent, Fee')
    eq_(app.iwin.import_table[0].amount_import, '-1000.00')
    eq_(app.iwin.import_table[1].transfer_import, 'Web Accept Payment Received, Fee')
    eq_(app.iwin.import_table[1].amount_import, '18.95')
...
def app_import_qif_with_empty_accounts():
    ...
    app = TestApp()
    app.drsel.set_date_range(YearRange(date(2007, 1, 1)))
    app.iwin = app.mw.parse_file_for_import(testdata.filepath('qif/empty_accounts.qif'))
    return app
...
def app_import_txns_with_low_day_fields():
    app = TestApp()
    app.iwin = app.fake_import('foo', LOW_DATE_FIELDS)
    return app
...
def test_switch_day_month(app):
    ...
    eq_(app.iwin.import_table[1].date_import, '02/01/2009')
    eq_(app.iwin.import_table[2].date_import, '01/12/2009')
...
def test_switch_day_year(app):
    ...
    # Dates are sorted after swap
    # Note: This is day/month/year
    eq_(app.iwin.import_table[0].date_import, '09/02/2001')
    eq_(app.iwin.import_table[1].date_import, '08/11/2005')
    eq_(app.iwin.import_table[2].date_import, '09/01/2012')
...
def app_import_txns_with_high_day_fields():
    app = TestApp()
    app.iwin = app.fake_import('foo', HIGH_DAY_FIELDS)
    return app
...
def app_import_txns_with_high_year_fields():
    app = TestApp()
    app.iwin = app.fake_import('foo', HIGH_YEAR_FIELDS)
    return app
...
def app_import_txn_with_31st_day_in_date():
    ...
    app = TestApp()
    app.iwin = app.fake_import('foo', [{'date': '31/07/2009', 'amount': '1'}])
    return app
...
def test_switch_apply_to_all(app):
    ...
    app.iwin.selected_pane_index = 2
    eq_(app.iwin.import_table[0].date_import, '02/01/2009') # not switched
